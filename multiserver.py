...
from flask import Flask, abort, make_response, render_template, url_for, request, jasonify
...
def create_app(config=None, config_file=None):
    ...
    # Add new endpoint to handle gaze data
    @app.route('/gaze', methods=['POST'])
    def gaze():
        data = request.json
        print(f"Gaze coordinates: x={data['x']}, y={data['y']}, zoom={data['zoom']}")
